...
__global__
void
separateChannels (const uchar4* const inputImageRGBA, int numRows, int numCols,
...
__global__
void
recombineChannels (const unsigned char* const redChannel,
...
void
allocateMemoryAndCopyToGPU (const size_t numRowsImage,
			    ...
			    const size_t filterWidth)
{
  ...
  checkCudaErrors(
      cudaMemcpy (d_filter, h_filter, sizeof(float) * filterWidth * filterWidth,
		  cudaMemcpyHostToDevice));

}

void
your_gaussian_blur (const uchar4 * const h_inputImageRGBA,
